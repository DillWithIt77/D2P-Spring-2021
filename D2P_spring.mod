...
var z{f in FEATURES, i in BRANCH_NODES} >= 0, <= 1, binary; # =1 if feature f goes left on branch k
...
# ADD CUTTING PLANES FROM PAPER
# THE FOLLOWING 4 CONSTRAINTS ALLOW US TO REMOVE THE PREVIOUS 6
# left and positive
subject to left_split_pos_cut {i in 1..SAMPLES_POS, k in BRANCH_NODES}:
sum{b in POS_LEAF_NODES} leaf_nodes_p_left[b, k]*c_pos[b,i] 
<= sum{j in FEATURES} a_pos[i, j]*z[j, k];

# right and positive
subject to right_split_pos_cut {i in 1..SAMPLES_POS, k in BRANCH_NODES}:
sum{b in POS_LEAF_NODES} leaf_nodes_p_right[b, k]*c_pos[b,i] 
<= 1 - sum{j in FEATURES} a_pos[i, j]*z[j, k];

# left and negative
subject to left_split_neg_cut {i in 1..SAMPLES_NEG, k in BRANCH_NODES}:
sum{b in NEG_LEAF_NODES} leaf_nodes_n_left[b, k]*c_neg[b,i] 
<= sum{j in FEATURES} a_neg[i, j]*z[j, k];

# right and negative
subject to right_split_neg_cut {i in 1..SAMPLES_NEG, k in BRANCH_NODES}:
sum{b in NEG_LEAF_NODES} leaf_nodes_n_right[b, k]*c_neg[b,i] 
<= 1 - sum{j in FEATURES} a_neg[i, j]*z[j, k];
